    ...
    <div>
      ...
      <div className="p-2.5 max-xsm:grid-cols-1 max-w-240 mx-auto gap-2.5 sm:p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
        {!isLoading &&
          ...
          gamesOnRange.map((game: Game) => {
            return <GameCard key={game.uuid} game={game} platform={platform} />;
          })}
        {isLoading && // Skeleton para feedback al usuario (funciona como otra grilla)
          new Array(50).fill(0).map((repeat, index) => {
...
const SkeletonCard = () => {
  return (
    <Card className="min-h-70 opacity-50">
      <CardContent>
        <Skeleton />
      </CardContent>
    </Card>
...
const GamesNotFound = ({ filter }: { filter: Filters["filter"] }) => {
  return (
